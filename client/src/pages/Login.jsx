...
const Login = () => {
  ...
  const onSubmitHandler = async (e)=>{
    try{
      //when we submit the form it prevent the webpage from reloading
      e.preventDefault(); 
      ...
      <div className='bg-slate-900 p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-300 text-sm'>
        ...
        <form onSubmit={onSubmitHandler} autoComplete='off'>
          {state === 'Sign Up' && (
            ...
            value={name} className='bg-transparent outline-none' type="text" placeholder='Full Name' 
            autoComplete='off'
            required/>
          </div>
        )}
          ...
          <p onClick={()=>navigate('/reset-password')} className='mb-4 text-indigo-500 cursor-pointer'>Forgot Passwoord?</p>
